...
`dependency_report()` function can generate a html report for the dependency analysis on the package:
...
is in the `Depends`/`Imports`/`LinkingTo` fields of package A, which means, package B 
is directly required for package A, denote `v1` as the total number of packages for package A, 
and denote `v2` as the total number of required packages if moving package
B to `Suggests` in package A (which means, now B is not enforced to be installed for package A). The
...
that package B brings in.

In the second scenario, if package B is in the `Suggests`/`Enhances` fields of package
A, now `v2` is the total number of required packages if moving package B to `Imports` in package A,
...
<script>
document.write('<img width="1000px" src="https://user-images.githubusercontent.com/449218/148210483-78c3a07c-114b-4216-bcd3-fca45d707de7.gif" />');
</script>
